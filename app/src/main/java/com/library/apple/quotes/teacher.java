...
public class teacher extends Fragment{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        myView = inflater.inflate(R.layout.teacher,container,false);

        MobileAds.initialize(getActivity(), "ca-app-pub-8750480772839804~5657654976");

        AdView adView = new AdView(getActivity());
        adView.setAdSize(AdSize.BANNER);
        adView.setAdUnitId("ca-app-pub-8750480772839804/3907984251");

        mAdView = myView.findViewById(R.id.adView_teacher);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);

        animationView = (LottieAnimationView)myView.findViewById(R.id.av_teacher);
        animationView.setAnimation("poi.json");
        animationView.playAnimation();
        animationView.setVisibility(View.VISIBLE);

        fab = (FloatingActionButton) myView.findViewById(R.id.fab_teacher);




        recyclerView=(RecyclerView)myView.findViewById(R.id.rv_teacher);
        ...
        myref= FirebaseDatabase.getInstance().getReference().child("teacher");
        myref.keepSynced(true);

        Query myMostViewedPostsQuery = myref.orderByChild("name");


        final FirebaseRecyclerAdapter<Contact,teacher.BlogViewHolder> recyclerAdapter=new FirebaseRecyclerAdapter<Contact,teacher.BlogViewHolder>(
                Contact.class,
                R.layout.row,
                teacher.BlogViewHolder.class,
                myMostViewedPostsQuery
        ) {

            @Override
            protected void populateViewHolder(final teacher.BlogViewHolder viewHolder, final Contact model, final int position) {
