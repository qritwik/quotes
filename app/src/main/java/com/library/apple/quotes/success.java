...
public class success extends Fragment{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        myView = inflater.inflate(R.layout.success,container,false);

        MobileAds.initialize(getActivity(), "ca-app-pub-8750480772839804~5657654976");

        AdView adView = new AdView(getActivity());
        adView.setAdSize(AdSize.SMART_BANNER);
        adView.setAdUnitId("ca-app-pub-8750480772839804/2588121586");

        mAdView = myView.findViewById(R.id.adView_success);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);


        animationView = (LottieAnimationView)myView.findViewById(R.id.av_success);
        animationView.setAnimation("poi.json");
        animationView.playAnimation();
        animationView.setVisibility(View.VISIBLE);

        fab = (FloatingActionButton) myView.findViewById(R.id.fab_success);


        recyclerView=(RecyclerView)myView.findViewById(R.id.rv_success);
        ...
        myref= FirebaseDatabase.getInstance().getReference().child("success");
        myref.keepSynced(true);

        Query myMostViewedPostsQuery = myref.orderByChild("name");


        final FirebaseRecyclerAdapter<Contact,success.BlogViewHolder> recyclerAdapter=new FirebaseRecyclerAdapter<Contact,success.BlogViewHolder>(
                Contact.class,
                R.layout.row,
                success.BlogViewHolder.class,
                myMostViewedPostsQuery
        ) {

            @Override
            protected void populateViewHolder(final success.BlogViewHolder viewHolder, final Contact model, final int position) {
