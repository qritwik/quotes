...
public class friend extends Fragment{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        myView = inflater.inflate(R.layout.friend,container,false);

        MobileAds.initialize(getActivity(), "ca-app-pub-8750480772839804~5657654976");

        AdView adView = new AdView(getActivity());
        adView.setAdSize(AdSize.BANNER);
        adView.setAdUnitId("ca-app-pub-8750480772839804/2215628637");

        mAdView = myView.findViewById(R.id.adView_friend);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);

        animationView = (LottieAnimationView)myView.findViewById(R.id.av_friend);
        animationView.setAnimation("poi.json");
        animationView.playAnimation();
        animationView.setVisibility(View.VISIBLE);

        fab = (FloatingActionButton) myView.findViewById(R.id.fab_friend);




        recyclerView=(RecyclerView)myView.findViewById(R.id.rv_friend);
        ...
        myref= FirebaseDatabase.getInstance().getReference().child("friend");
        myref.keepSynced(true);

        Query myMostViewedPostsQuery = myref.orderByChild("name");


        final FirebaseRecyclerAdapter<Contact,friend.BlogViewHolder> recyclerAdapter=new FirebaseRecyclerAdapter<Contact,friend.BlogViewHolder>(
                Contact.class,
                R.layout.row,
                friend.BlogViewHolder.class,
                myMostViewedPostsQuery
        ) {

            @Override
            protected void populateViewHolder(final friend.BlogViewHolder viewHolder, final Contact model, final int position) {
