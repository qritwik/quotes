...
public class relationship extends Fragment{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        myView = inflater.inflate(R.layout.relationship,container,false);

        MobileAds.initialize(getActivity(), "ca-app-pub-8750480772839804~5657654976");

        AdView adView = new AdView(getActivity());
        adView.setAdSize(AdSize.SMART_BANNER);
        adView.setAdUnitId("ca-app-pub-8750480772839804/2942279145");

        mAdView = myView.findViewById(R.id.adView_relationship);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);

        animationView = (LottieAnimationView)myView.findViewById(R.id.av_relationship);
        animationView.setAnimation("poi.json");
        animationView.playAnimation();
        animationView.setVisibility(View.VISIBLE);

        fab = (FloatingActionButton) myView.findViewById(R.id.fab_relationship);




        recyclerView=(RecyclerView)myView.findViewById(R.id.rv_relationship);
        ...
        myref= FirebaseDatabase.getInstance().getReference().child("relationship");
        myref.keepSynced(true);

        Query myMostViewedPostsQuery = myref.orderByChild("name");


        final FirebaseRecyclerAdapter<Contact,relationship.BlogViewHolder> recyclerAdapter=new FirebaseRecyclerAdapter<Contact,relationship.BlogViewHolder>(
                Contact.class,
                R.layout.row,
                relationship.BlogViewHolder.class,
                myMostViewedPostsQuery
        ) {

            @Override
            protected void populateViewHolder(final relationship.BlogViewHolder viewHolder, final Contact model, final int position) {
                ...
                viewHolder.mView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        relationship.this.startActivity(intent);
